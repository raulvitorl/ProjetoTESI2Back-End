...
class BancosConsulta extends JFrame {
	...
	BancosConsulta(JFrame framePai, Conexao conexao){ // método construtor
		super("Consulta Bancos"); // chamando construtor da classe mãe
		...
		bancosCadastro = new BancosCadastro(this, cnx);
	...
	class AcaoSair extends AbstractAction{
		...
		public void actionPerformed(ActionEvent e) {
			...
			BancosConsulta.this.setVisible(false);
			pai.setVisible(true);
	...
	public void buscar() {
		...
		try {
			if(fldValor.getText().equals("")) {
				...
				case 0:
					try {
						listaDeBancos.add(bl.getBanco(Integer.parseInt(fldValor.getText())));
					} catch (NumberFormatException e) {
						JOptionPane.showMessageDialog(null, "Entrada inv�lida para o filtro selecionado");
					}
					break;
				case 1:
					listaDeBancos = bl.getBancosPorNome(fldValor.getText());
					break;
				}

			}
			
			tblQuery.setModel(new BancosTableModel(listaDeBancos));
			btnEditar.setEnabled(false);
			btnExcluir.setEnabled(false);
				...
				EntityNotExistException e) 
		{
			JOptionPane.showMessageDialog(this, e.getMessage(), 
					"Consulta de Banco", JOptionPane.ERROR_MESSAGE);
		}
