...
class FornecedoresConsulta extends JFrame {

	private Conexao cnx = null;
	Mercado pai;
	FornecedoresLogic fl;

	FornecedorCadastro fornecedorCadastro;
	...
	FornecedoresConsulta(JFrame framePai, Conexao conexao){ // método construtor
		super("Consulta de Fornecedor"); // chamando construtor da classe mãe
		...
		fornecedorCadastro = new FornecedorCadastro(this, cnx);
		...
		cmbChaves = new JComboBox(new String[] {"Codigo","Nome Fantasia", "CNPJ","Municipio"});
	...
	class AcaoEditar extends AbstractAction{
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				fornecedorCadastro.editar(codigo);
	...
	class AcaoExcluir extends AbstractAction{
		...
		public void actionPerformed(ActionEvent e) {
			...
			codigo = (int) tblQuery.getValueAt(tblQuery.getSelectedRow(), 0);
			try {
				fornecedorCadastro.excluir(codigo);
			} catch (EntityTableIsEmptyException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
	...
	class AcaoSair extends AbstractAction{
		...
		AcaoSair(){
			...
			putValue(SHORT_DESCRIPTION, 
					"Fecha consulta de fornecedores!");
			putValue(SMALL_ICON, 
		...
		public void actionPerformed(ActionEvent e) {
			...
			FornecedoresConsulta.this.setVisible(false);
			pai.setVisible(true);
	...
	public void buscar() {
		...
		try {
			...
			if(fldValor.getText().equals("")) {
				listaDeFornecedores = fl.getFornecedores();
			}else{
				switch (cmbChaves.getSelectedIndex()) {
				case 0:
					try {
						busca = Integer.parseInt(fldValor.getText());
						listaDeFornecedores.add(fl.getFornecedor(busca));
					...
					break;
				case 1:
					listaDeFornecedores = fl.getFornecedoresPorNomeFantasia(fldValor.getText());
					break;
				case 2:
					listaDeFornecedores = fl.getFornecedoresPorCnpj(fldValor.getText());
					break;
				case 3:
					listaDeFornecedores = fl.getFornecedoresPorMunicipio(fldValor.getText());
					break;
				}

			}

			tblQuery.setModel(new FornecedoresTableModel(listaDeFornecedores));
			btnEditar.setEnabled(false);
			btnExcluir.setEnabled(false);
				...
				EntityNotExistException e) 
		{
			JOptionPane.showMessageDialog(this, e.getMessage(), 
					"Consulta de fornecedor", JOptionPane.ERROR_MESSAGE);
		}
