...
class AtendentesConsulta extends JFrame {

	private Conexao cnx = null;
	Mercado pai;
	AtendentesLogic al;

	AtendentesCadastro atendentesCadastro;
	...
	AtendentesConsulta(JFrame framePai, Conexao conexao){ // método construtor
		super("Consulta de Atendentes"); // chamando construtor da classe mãe
		...
		atendentesCadastro = new AtendentesCadastro(this, cnx);
	...
	class AcaoEditar extends AbstractAction{
		...
		public void actionPerformed(ActionEvent e) {
			...
			codigo =  (int) tblQuery.getValueAt(tblQuery.getSelectedRow(), 0);
				atendentesCadastro.editar(codigo);
	...
	class AcaoExcluir extends AbstractAction{
		...
		public void actionPerformed(ActionEvent e) {
			...
			int codigo;
			codigo =  (int) tblQuery.getValueAt(tblQuery.getSelectedRow(), 0);
			atendentesCadastro.excluir(codigo);
	...
	class AcaoSair extends AbstractAction{
		...
		public void actionPerformed(ActionEvent e) {
			...
			AtendentesConsulta.this.setVisible(false);
			pai.setVisible(true);
	...
	public void buscar() {
		...
		try {
			if(fldValor.getText().equals("")) {
				...
				case 0:
					try {
						listaDeAtendentes.add(al.getAtendente(Integer.parseInt(fldValor.getText())));
					...
					break;
				case 1:
					listaDeAtendentes = al.getAtendentesPorNome(fldValor.getText());
					break;
				}

			}
			
			tblQuery.setModel(new AtendentesTableModel(listaDeAtendentes));
			btnEditar.setEnabled(false);
			btnExcluir.setEnabled(false);
				...
				EntityTableIsEmptyException e) 
		{
			JOptionPane.showMessageDialog(this, e.getMessage(), 
					"Consulta de Atendente", JOptionPane.ERROR_MESSAGE);
		}
