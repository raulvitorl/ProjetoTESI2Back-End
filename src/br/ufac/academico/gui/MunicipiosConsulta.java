...
class MunicipiosConsulta extends JFrame {
	...
	MunicipiosConsulta(JFrame framePai, Conexao conexao){ // método construtor
		super("Consulta de Municipios"); // chamando construtor da classe mãe
		...
		municipiosCadastro = new MunicipiosCadastro(this, cnx);
		...
		cmbChaves = new JComboBox(new String[] {"Codigo", "Nome","UF"});
	...
	class AcaoEditar extends AbstractAction{
		...
		public void actionPerformed(ActionEvent e) {
			...
			municipiosCadastro.editar(codigo);
	...
	class AcaoExcluir extends AbstractAction{
		...
		public void actionPerformed(ActionEvent e) {
			...
			int codigo;
			codigo =  (int) tblQuery.getValueAt(tblQuery.getSelectedRow(), 0);
			municipiosCadastro.excluir(codigo);
	...
	class AcaoSair extends AbstractAction{
		...
		public void actionPerformed(ActionEvent e) {
			...
			MunicipiosConsulta.this.setVisible(false);
			pai.setVisible(true);
	...
	public void buscar() {
		...
		try {
			if(fldValor.getText().equals("")) {
				listaDeMunicipios = ml.getMunicipios();
			}else{
				switch (cmbChaves.getSelectedIndex()) {
				case 0:
					try {
						listaDeMunicipios.add(ml.getMunicipio(Integer.parseInt(fldValor.getText())));
					} catch (NumberFormatException e) {
						JOptionPane.showMessageDialog(null, "Entrada inv�lida para o filtro selecionado");
					}
					break;
				case 1:
					listaDeMunicipios = ml.getMunicipiosPorNome(fldValor.getText());
					break;
				case 2:
					listaDeMunicipios = ml.getMunicipiosPorUF(fldValor.getText());
					break;
				}

			}
			
			tblQuery.setModel(new MunicipiosTableModel(listaDeMunicipios));
			btnEditar.setEnabled(false);
			btnExcluir.setEnabled(false);
				...
				EntityNotExistException e) 
		{
			JOptionPane.showMessageDialog(this, e.getMessage(), 
					"Consulta de Municipio", JOptionPane.ERROR_MESSAGE);
		}
