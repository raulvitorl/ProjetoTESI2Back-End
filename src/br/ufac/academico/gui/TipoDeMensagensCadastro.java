...
class TipoDeMensagensCadastro extends JFrame {
	...
	private int acao;
	private TiposMensagensConsulta pai;
	private Conexao cnx;
	private ResultSet rs;
	private TiposMensagensLogic bl;

	private JPanel pnlControles, pnlOperacoes, pnlRotulos, pnlCampos;
	private JComboBox cmbCentro;
	private JTextField fldTmsCodigo, fldTmsDescricao;
	...
	TipoDeMensagensCadastro(JFrame framePai, Conexao conexao){ // método construtor
		super(""); // chamando construtor da classe mãe
		setSize(800, 600);				// definindo dimensões da janela		
		setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);
		setLocationRelativeTo(null);
		pai = (TiposMensagensConsulta)framePai;
		cnx = conexao;
		...
		fldTmsCodigo = new JTextField();
		fldTmsDescricao = new JTextField();		
		
		
		pnlCampos = new JPanel(new GridLayout(3,1,5,5));
		pnlCampos.add(fldTmsCodigo);
		pnlCampos.add(fldTmsDescricao);
	...
	class AcaoConfirmar extends AbstractAction{
		...
		public void actionPerformed(ActionEvent e) {
			try {
				switch (acao) {
				case INCLUSAO:
					bl.addTipoMensagem(Integer.parseInt(fldTmsCodigo.getText()),fldTmsDescricao.getText());
					break;
				case EDICAO:
					bl.updTipoMensagens(Integer.parseInt(fldTmsCodigo.getText()),fldTmsDescricao.getText());
					break;
				case EXCLUSAO:
					bl.delTipoMensagem(Integer.parseInt(fldTmsCodigo.getText()),fldTmsDescricao.getText());
					break;
				}
				limparCampos();
				TipoDeMensagensCadastro.this.setVisible(false);
				pai.setVisible(true);
				pai.buscar();				
			...
			{
				JOptionPane.showMessageDialog(TipoDeMensagensCadastro.this, e1.getMessage(), 
	...
	public void incluir() {
		...
		fldTmsCodigo.setEnabled(true);
		fldTmsDescricao.setEnabled(true);
	...
	public void excluir(int codigo) {
		...
		fldTmsCodigo.setEnabled(false);
		fldTmsDescricao.setEnabled(false);
	...
	public void limparCampos() {
		fldTmsCodigo.setText("");
		fldTmsDescricao.setText("");
	...
	public void carregarCampos(int codigo) {
		...
		try {
			...
			fldTmsDescricao.setText(String.valueOf(tms.getTmsDescricao()));
		} catch (DataBaseGenericException | 
				DataBaseNotConnectedException | 
